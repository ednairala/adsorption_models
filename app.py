...
def build_plotly_figure(model_name, Ce_exp, qe_exp, result,
    ...
    if show_ci and result.get('covariance') is not None:
        pcov   = np.asarray(result['covariance'], dtype=float)
        pnames = list(result['parameters'].keys())
        pvals  = np.array([result['parameters'][k] for k in pnames], dtype=float)
...
def generate_fixedbed_python_script(t_exp, cc0_exp, all_results,
                                    C0_col, Q_col, Z_col, m_col):
    """Generate a self-contained Python script for fixed-bed breakthrough fitting."""
    t_list = ", ".join(f"{v}" for v in t_exp)
    cc0_list = ", ".join(f"{v}" for v in cc0_exp)

    param_sections = []
    for name, res in sorted(all_results.items(), key=lambda x: x[1]['aic']):
        param_str = ", ".join(f"{k}={v:.6g}" for k, v in res['parameters'].items())
        param_sections.append(
            f"    # {name}: R²={res['r_squared']:.5f}, RMSE={res['rmse']:.5f}, AIC={res['aic']:.3f}\\n"
            f"    # Parameters: {param_str}"
    ...
    return f'''"""
Fixed-Bed Breakthrough Model Fitting - Auto-generated script
=============================================================
Generated by Adsorption Library (Streamlit app)
...
# Experimental data
t_exp = np.array([{t_list}], dtype=float)
cc0_exp = np.array([{cc0_list}], dtype=float)
...
# Model definitions
def bohart_adams(t, kBA, N0):
    exponent = kBA * N0 * Z / Q - kBA * C0 * t
    return 1.0 / (1.0 + np.exp(exponent))


def thomas(t, kTh, qTh):
...
def wolborska(t, beta, N0):
    exponent = (beta * C0 / N0) * (t - N0 * Z / (C0 * Q))
    return np.clip(np.exp(exponent), 0.0, 1.0)


MODELS = {{
    "Bohart-Adams": (bohart_adams, [1e-4, 1000.0], ([1e-8, 1.0], [10.0, 1e7])),
    "Thomas": (thomas, [1e-3, 10.0], ([1e-8, 0.01], [10.0, 1e5])),
    "Yoon-Nelson": (yoon_nelson, [0.01, np.median(t_exp)], ([1e-6, 0.0], [10.0, np.inf])),
    "Wolborska": (wolborska, [0.01, 1000.0], ([1e-8, 1.0], [10.0, 1e7])),
}}
...
for name, (fn, p0, bounds) in MODELS.items():
    try:
        popt, _ = curve_fit(fn, t_exp, cc0_exp, p0=p0, bounds=bounds, maxfev=30000)
        y_pred = fn(t_exp, *popt)
        r2, rmse, aic = calc_metrics(cc0_exp, y_pred, len(popt))
        results[name] = dict(fn=fn, popt=popt, r2=r2, rmse=rmse, aic=aic)
        print(f"{{name:12s}}  R²={{r2:.5f}}  RMSE={{rmse:.5f}}  AIC={{aic:.3f}}")
    except Exception as e:
...
# Plot
if results:
    t_smooth = np.linspace(np.min(t_exp), np.max(t_exp), 500)
    colors = {{
        "Bohart-Adams": "#534AB7",
        "Thomas": "#1f77b4",
        "Yoon-Nelson": "#2ca02c",
    ...
    plt.figure(figsize=(9, 6))
    plt.scatter(t_exp, cc0_exp, color="#d62728", s=65, label="Experimental", zorder=5)
    for name, res in sorted(results.items(), key=lambda x: x[1]["aic"]):
        y_s = res["fn"](t_smooth, *res["popt"])
        plt.plot(t_smooth, y_s, lw=2.3, color=colors.get(name, None),
                 label=f"{{name}} (AIC={{res['aic']:.2f}})")

    plt.xlabel("Time (min)")
    plt.ylabel("C/C0")
    plt.title("Fixed-Bed Breakthrough Model Comparison")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.ylim(-0.05, 1.1)
    plt.tight_layout()
    plt.savefig("fixedbed_breakthrough_comparison.png", dpi=150)
    plt.show()
    print("\\nPlot saved to fixedbed_breakthrough_comparison.png")
else:
    print("No model converged.")
'''


def generate_fixedbed_pdf_report(t_exp, cc0_exp, all_results,
                                 C0_col, Q_col, Z_col, m_col, alpha=0.05):
    """Generate a PDF report for fixed-bed breakthrough model results."""
    from reportlab.lib.pagesizes import A4
    from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
    from reportlab.lib.units import cm
    from reportlab.lib import colors
    ...
    title_style = ParagraphStyle(
        'FB_Title', parent=styles['Title'], fontSize=18,
        spaceAfter=6, textColor=colors.HexColor('#1a1a2e')
    )
    h1_style = ParagraphStyle(
        'FB_H1', parent=styles['Heading1'], fontSize=13,
        textColor=colors.HexColor('#534AB7'), spaceAfter=4
    )
    h2_style = ParagraphStyle(
        'FB_H2', parent=styles['Heading2'], fontSize=11,
        textColor=colors.HexColor('#0F6E56'), spaceAfter=2
    )
    small_style = ParagraphStyle('FB_Small', parent=styles['Normal'], fontSize=8)

    story = []
    story.append(Paragraph("Fixed-Bed Breakthrough Models - Results Report", title_style))
    story.append(Paragraph(
        f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')} | "
        f"C0={C0_col} mg/L | Q={Q_col} L/min | Z={Z_col} cm | m={m_col} g | α = {alpha}",
        small_style
    ))
    story.append(HRFlowable(width="100%", thickness=1.5, color=colors.HexColor('#534AB7')))
    story.append(Spacer(1, 12))
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph("Experimental Data", h1_style))
    exp_data = [['t (min)', 'C/C0']] + [[f"{t:.6g}", f"{c:.6g}"] for t, c in zip(t_exp, cc0_exp)]
    et = Table(exp_data, colWidths=[5*cm, 5*cm])
    et.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#444444')),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.white),
        ...
        if not st.session_state.get('eq_ready', False):
            ...
            for i, mname in enumerate(models_to_plot):
                ...
                if show_ci and res.get('covariance') is not None:
                    ...
                    if not cov_valid:
                        eq_ci_skipped_models.append(mname)
                        continue
            ...
            if show_ci and eq_ci_skipped_models:
                skipped = ", ".join(sorted(set(eq_ci_skipped_models)))
                st.caption(
...
def page_kinetics():
    ...
    with st.sidebar:
        ...
        if system_type == "Batch (discontinuous)":
            rank_metric_kin = st.radio(
                "Best-model criterion",
                ["AIC", "R²"],
                horizontal=True,
                help="AIC: lower is better. R²: higher is better."
            )
        st.divider()
        if system_type == "Batch (discontinuous)":
            st.subheader("Plot options")
    ...
    if system_type == "Batch (discontinuous)":
        _kinetics_batch(alpha, show_ci_kin, rank_metric_kin)
    else:
...
def _kinetics_batch(alpha, show_ci_kin=True, rank_metric="AIC"):
...
Time in minutes, qt in mg/g, C_bulk in mg/L.
    ...
    with tab_reaction:
        if not st.session_state.get('kin_batch_ready', False):
            ...
            # Best model summary using selected ranking criterion
            if rank_metric == "R²":
                best_model = max(results.items(), key=lambda x: x[1].get('r_squared', -np.inf))[0]
                best_label = "Best model (R²)"
            else:
                best_model = min(results.items(), key=lambda x: x[1].get('aic', np.inf))[0]
                best_label = "Best model (AIC)"
            best_res = results[best_model]

            bc1, bc2, bc3 = st.columns(3)
            bc1.metric(best_label, best_model)
...
- **BDST** (Bed Depth Service Time): linear scale-up relationship between bed height and service time
    ...
    with tab_bt:
        if not st.session_state.get('kin_fb_ready', False):
            ...
            with ex2:
                fb_py = generate_fixedbed_python_script(
                    t_bt, CC0, bt_results,
                    C0_col=C0_col, Q_col=Q_col, Z_col=Z_col, m_col=m_col
                )
                st.download_button(
                    "⬇ Python script (.py)",
                    data=fb_py,
                    file_name="fixedbed_breakthrough_models.py",
                    mime="text/plain"
                )
            with ex3:
                fb_pdf = generate_fixedbed_pdf_report(
                    t_bt, CC0, bt_results,
                    C0_col=C0_col, Q_col=Q_col, Z_col=Z_col, m_col=m_col,
                    alpha=alpha
                )
                st.download_button(
                    "⬇ PDF report",
                    data=fb_pdf,
                    file_name="fixedbed_breakthrough_report.pdf",
                    mime="application/pdf"
                )
